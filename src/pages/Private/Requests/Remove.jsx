...
const ModalRemove = ({ modal, toggle, request, localRefetchData }) => {
    ...
    const handleRemove = async () => {
        if (request) {
            setIsLoading(true);
            try {
                await API.delete(`/persons/${request.id}`);
        ...
        <Modal isOpen={modal} toggle={toggle} style={{ color: "var(--insignio-primary-text)" }}>
            <div className="">
                ...
                <div className="text-center">
                    ...
                    <span className="d-block mt-2 mb-0 fw-bold" style={{ fontSize: 16 }}>Deletar Requisição</span>
                </div>
            </div>
            <ModalBody className="text-center">
                <p className="mb-1">Tem certeza de que deseja excluir:</p>
                <p className=" mb-4 fw-bold">Requisição: {request.id}</p>
