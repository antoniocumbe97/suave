...
const ModalCreate = ({ modal, toggle, refetchData }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
         ...
         //   console.log("response", response);;
            ...
            const response = await API.post('/users/godson', formData);
            const iconHtml = renderToString(<FaCheckCircle color="#4CAF50" size={70} style={{ border: "none" }} />);
         //   console.log("response", response);
            refetchData();
            toggle();
            Swal.fire({
                title: "Recruta Registado Com Sucesso!",
                iconHtml: iconHtml
            });
            ...
            if (error.code === 'ERR_NETWORK') {
                ...
                Swal.fire({
                    ...
                    text: 'Houve um problema ao conectar ao servidor.'
                });
            }
        } finally {
            setIsLoading(false);
    ...
    return (
        <Modal isOpen={modal} toggle={toggle} size="lg" style={{ color: "var(--insignio-primary-text)" }} centered>
            <ModalHeader toggle={toggle} className={'border-bottom text-center pb-3'}>Registar Recruta</ModalHeader>
            <ModalBody>
                <Row>
                    <Col md={12}>
                        <FormGroup>
