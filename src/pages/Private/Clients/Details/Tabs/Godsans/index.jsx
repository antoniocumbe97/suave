...
const Godsons = ({ setRecruits }) => {
    ...
    useEffect(() => {
        const load = async () => {
            setIsLoading(true);
            try {
                const response = await API.get(`users/${id}/godsons`);
                // console.log('Response', response.data)
                if (Array.isArray(response.data)) {
                    setData(response.data);
                    setRecruits(response.data.length)
        ...
        <Fragment>
            ...
            <Table responsive hover className='mb-0'>
                ...
                {isLoading ?
                    <tbody>
                        <tr>
                            <td colSpan={5} className="px-0 pt-3 pb-0">
                                <IsLoading />
                            </td>
                        </tr>
                    </tbody>
                    ...
                    <>
                        {data.length === 0 ?
                            <tbody>
                                <tr>
                                    <td colSpan={5} className="text-center text-dark py-3">Recrutas não encontrados</td>
                                </tr>
                            </tbody>
                            :
                            <>
                                        ...
                                        return (
                                            <tr key={index} className="border-start-0 border-end-0" style={{ border: "2px solid #D9D9D9", color: "var(--insignio-primary-text)" }}>
                                                ...
                                                <td className="d-flex justify-content-end gap-3 border-0">
                                                    <Link
                                                        to={SYSTEM_ROUTES.GODSONS.show + item.id}
